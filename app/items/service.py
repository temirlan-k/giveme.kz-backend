...
class ItemService:
    async def create_item(
        ...
        if not current_user.get("is_active"):
            raise HTTPException(
                status_code=403, detail="You should active your account"
            )
        if not category:
            raise HTTPException(status_code=400, detail="Category does not exist.")

        try:
            s3_db_key = await save_file_locally(item_file, "item_photos")
    ...
    async def get_items_by_category(
        ...
        if category_names:
            category_names_upper = [
                category_name.upper() for category_name in category_names
            ]

            categories = (
                db.query(Category).filter(Category.name.in_(category_names_upper)).all()
            )
            category_id_map = {
                category.name.upper(): category.id for category in categories
            }
            ...
            for category_name in category_names:
                ...
                items_with_category_name.extend(
                    {**item.__dict__, "cat_name": category_name.upper()}
                    for item in items
                )
        else:
            items = query.all()
            items_with_category_name.extend(
                {
                    ...
                    "cat_name": db.query(Category.name)
