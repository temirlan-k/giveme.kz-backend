...
from fastapi import APIRouter, BackgroundTasks, Body,Depends, File, Query ,Header, UploadFile
from sqlalchemy.orm import Session

from app.auth.auth_bearer import JWTBearer
from app.config.db import get_db
from app.users.schemas import CurrentUserOut, ResetForgetPassword, UserCreate, UserLogin,ForgetPasswordRequest
...
async def forget_password(email:ForgetPasswordRequest = Body(...),db: Session = Depends(get_db)):
    return await UserService.forget_password(email,db)

@router.post('/reset_password/',tags=['auth'])
async def reset_password(token:str=Header(None),rfp:ResetForgetPassword = Body(...),db: Session = Depends(get_db)):
    return await UserService.reset_password(token,rfp,db)



@router.get('/me',dependencies=[Depends(JWTBearer())],tags=['users'])
async def read_current_user(current_user:CurrentUserOut = Depends(UserService.get_current_user)):
    return current_user
